...
-- Final integration bootstrap: preserve the complete Margaris cultural/business/deals
-- payload while exposing freshness timestamps required by the Mavroeidis map
-- activity, area-intelligence and discovery layers. Forward-only and safe for
-- databases that previously applied either branch's bootstrap replacement.
      ...
      coalesce((
        ...
        from (
          select target_type, place_id, post_id, route_id, cultural_event_id, author_name,
            author_kind, user_id, profile_id, posting_identity, text, created_at, sort_order
